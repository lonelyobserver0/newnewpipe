...
import static org.newnewpipe.app.QueueItemMenuUtil.openPopupMenu;
import static org.newnewpipe.extractor.ServiceList.YouTube;
import static org.newnewpipe.extractor.utils.Utils.isNullOrEmpty;
import static org.newnewpipe.app.ktx.ViewUtils.animate;
import static org.newnewpipe.app.ktx.ViewUtils.animateRotation;
import static org.newnewpipe.app.player.PlayerService.*;
import static org.newnewpipe.app.player.helper.PlayerHelper.*;
import static org.newnewpipe.app.player.helper.PlayerHelper.MinimizeMode.MINIMIZE_ON_EXIT_MODE_BACKGROUND;
import static org.newnewpipe.app.player.helper.PlayerHelper.MinimizeMode.MINIMIZE_ON_EXIT_MODE_NONE;
import static org.newnewpipe.app.player.helper.PlayerHelper.MinimizeMode.MINIMIZE_ON_EXIT_MODE_POPUP;
...
import org.newnewpipe.app.casting.CastQueueEntry;
import org.newnewpipe.app.casting.CastQueueInput;
import org.newnewpipe.app.casting.CastQueueMapper;
import org.newnewpipe.app.casting.ChromecastCastManager;
import org.newnewpipe.app.casting.DlnaDevice;
import org.newnewpipe.app.casting.DlnaDiscoveryManager;
import org.newnewpipe.app.casting.DlnaStreamPicker;
import org.newnewpipe.app.database.stream.model.StreamEntity;
import org.newnewpipe.app.databinding.PlayerBinding;
import org.newnewpipe.app.databinding.PlayerPopupCloseOverlayBinding;
...
import org.newnewpipe.app.music.LastFmScrobbler;
import org.newnewpipe.app.music.LrcLine;
...
import org.newnewpipe.app.watchtogether.PlaybackSnapshot;
import org.newnewpipe.app.watchtogether.WatchSyncAction;
import org.newnewpipe.app.watchtogether.WatchSyncEngine;
import org.newnewpipe.app.watchtogether.WatchTogetherClient;
import org.newnewpipe.app.watchtogether.WatchTogetherProtocol;
import org.newnewpipe.app.watchtogether.WatchTogetherRoomCode;
import org.newnewpipe.app.watchtogether.WatchTogetherServer;
import org.newnewpipe.app.watchtogether.WatchTogetherSession;
import org.newnewpipe.extractor.*;
import org.newnewpipe.extractor.exceptions.ExtractionException;
import org.newnewpipe.extractor.sponsorblock.SponsorBlockAction;
import org.newnewpipe.extractor.sponsorblock.SponsorBlockSegment;
import org.newnewpipe.extractor.stream.*;
import org.newnewpipe.app.fragments.OnScrollBelowItemsListener;
import org.newnewpipe.app.fragments.detail.VideoDetailFragment;
import org.newnewpipe.app.info_list.StreamSegmentAdapter;
...
import org.newnewpipe.app.player.bulletComments.MovieBulletCommentsPlayer;
import org.newnewpipe.app.player.event.DisplayPortion;
import org.newnewpipe.app.player.event.PlayerEventListener;
import org.newnewpipe.app.player.event.PlayerGestureListener;
import org.newnewpipe.app.player.event.PlayerServiceEventListener;
import org.newnewpipe.app.player.helper.AudioReactor;
import org.newnewpipe.app.player.helper.CustomRenderersFactory;
...
import org.newnewpipe.app.player.helper.MediaSessionManager;
import org.newnewpipe.app.player.helper.PlayerDataSource;
import org.newnewpipe.app.player.helper.PlayerHelper;
import org.newnewpipe.app.player.datasource.SabrSessionStore;
import org.newnewpipe.app.player.listeners.view.PlaybackSpeedClickListener;
import org.newnewpipe.app.player.listeners.view.QualityClickListener;
...
import org.newnewpipe.app.player.playback.MediaSourceManager;
import org.newnewpipe.app.player.playback.PlaybackListener;
import org.newnewpipe.app.player.playback.PlayerMediaSession;
import org.newnewpipe.app.player.playback.SurfaceHolderCallback;
import org.newnewpipe.app.player.playqueue.PlayQueue;
import org.newnewpipe.app.player.playqueue.PlayQueueAdapter;
import org.newnewpipe.app.player.playqueue.PlayQueueItem;
import org.newnewpipe.app.player.playqueue.PlayQueueItemBuilder;
import org.newnewpipe.app.player.playqueue.PlayQueueItemHolder;
import org.newnewpipe.app.player.playqueue.PlayQueueItemTouchCallback;
...
import org.newnewpipe.app.player.resolver.VideoPlaybackResolver.SourceType;
import org.newnewpipe.app.player.seekbarpreview.SeekbarPreviewThumbnailHelper;
import org.newnewpipe.app.player.seekbarpreview.SeekbarPreviewThumbnailHolder;
import org.newnewpipe.app.sleep.SleepTimerService;
import org.newnewpipe.app.util.*;
import org.newnewpipe.app.util.external_communication.KoreUtils;
import org.newnewpipe.app.util.external_communication.ShareUtils;
import org.newnewpipe.app.views.ExpandableSurfaceView;
import org.newnewpipe.app.views.player.PlayerFastSeekOverlay;
...
public final class Player implements
    ...
    /** Scrobbling Last.fm (piano 022-S12), lazy-init al primo tick di progress. */
    @Nullable private LastFmScrobbler lastFmScrobbler;

    // Overlay testi sincronizzati (022-S11)
    @Nullable private LyricsOverlay lyricsOverlay;
    @Nullable private LyricsClient lyricsClient;
    @Nullable private Disposable lyricsFetchDisposable;
    @Nullable private String cachedLyricsUrl;

    /** Dialogo cast DLNA (piano 022-S2): ricostruito a ogni aggiornamento della lista dispositivi. */
    @Nullable private AlertDialog castDialog;
    private boolean rebuildingCastDialog = false;

    /** Cast Chromecast (piano 022-S3): facade Cast SDK, gate runtime su Play Services. */
    @Nullable private ChromecastCastManager chromecastManager;

    /** Watch together (piano 022-S16): sessione attiva creata/unitasi dal player. */
    @Nullable private WatchTogetherSession watchTogetherSession;
    @Nullable private AlertDialog watchTogetherDialog;
    ...
    private void initListeners() {
        ...
        binding.watchTogetherButton.setOnClickListener(v -> onWatchTogetherClicked());
        binding.watchTogetherLeaveButton.setOnClickListener(v -> leaveWatchTogetherRoom());
        initLyricsOverlay();
        binding.lyricsButton.setOnClickListener(v -> onLyricsClicked());
        initChromecast();
...
//        }
    ...
    public void destroy() {
        ...
        // Watch together (022-S16): esce dalla stanza e ferma il server
        // embedded se questo dispositivo era l'host della stanza.
        stopWatchTogetherSession();

        databaseUpdateDisposable.clear();
        progressUpdateDisposable.set(null);
        if (lyricsFetchDisposable != null && !lyricsFetchDisposable.isDisposed()) {
            lyricsFetchDisposable.dispose();
        }
    ...
    private void setupElementsVisibility() {
        if (popupPlayerSelected()) {
            ...
            binding.castButton.setVisibility(View.GONE);
            binding.chromecastButton.setVisibility(View.GONE);
            binding.watchTogetherButton.setVisibility(View.GONE);
            ...
            binding.castButton.setVisibility(View.VISIBLE);
            binding.chromecastButton.setVisibility(ChromecastCastManager.isPlayServicesAvailable(context)
                    ? View.VISIBLE : View.GONE);
            binding.watchTogetherButton.setVisibility(View.VISIBLE);
    ...
    private void triggerProgressUpdate(final boolean isRewind,
        ...
        // Last.fm scrobbling (piano 022-S12): il tick periodico della progress bar
        // alimenta lo scrobbler (soglia 50%/4min, fire-once per brano in ScrobbleTracker).
        // No-op se lo scrobbling non è abilitato nelle prefs.
        if (currentItem != null) {
            if (lastFmScrobbler == null) {
                lastFmScrobbler = LastFmScrobbler.get(context);
            }
            lastFmScrobbler.onTrackChanged(currentItem.getTitle(), currentItem.getUploader(),
                    currentItem.getDuration() * 1000L);
            lastFmScrobbler.onPosition(currentProgress);
        }

        // Overlay testi sincronizzati (022-S11): il tick di progresso alimenta
        // la riga corrente (no-op se l'overlay è chiuso o senza righe).
        if (lyricsOverlay != null && lyricsOverlay.isVisible()) {
            lyricsOverlay.updatePosition(currentProgress);
        }

        // Watch together (022-S16): il tick alimenta il broadcast host
        // (throttlato nel WatchTogetherSession; no-op se non in sessione/host).
        publishWatchTogetherTick();
...
case ERROR_CODE_DECODER_INIT_FAILED: {
    ...
    public void onPlaybackSynchronize(@NonNull final PlayQueueItem item, final boolean wasBlocked) {
        ...
        currentItem = item;
        onLyricsTrackChanged();
        // Watch together (022-S16): cambio traccia → snapshot immediato
        // (la mediaUrl diversa supera il throttle nel WatchSyncEngine).
        publishWatchTogetherTick();
    ...
    public void seekTo(final long positionMillis) {
        ...
        if (!exoPlayerIsNull()) {
            ...
            publishWatchTogetherSeek(normalizedPositionMillis);
        }
    }
    ...
    /*//////////////////////////////////////////////////////////////////////////
    // Testi sincronizzati (022-S11)
    //////////////////////////////////////////////////////////////////////////*/
    //region Lyrics overlay

    private void initLyricsOverlay() {
        lyricsOverlay = new LyricsOverlay(binding.lyricsOverlayRoot, binding.lyricsScrollView,
    ...
    public void onLyricsClicked() {
        ...
        if (!currentItem.getUrl().equals(cachedLyricsUrl)) {
            fetchLyrics(currentItem);
        } else if (lyricsOverlay.getHasLines()) {
            if (!exoPlayerIsNull()) {
                lyricsOverlay.updatePosition(Math.max((int) simpleExoPlayer.getCurrentPosition(), 0));
            }
        }
        // cachedLyricsUrl == url corrente: lo stato (errore/assente) è già
        // renderizzato nell'overlay — niente refetch a ogni toggle.
    }

    /** Cambio brano: invalida la cache e, se l'overlay è aperto, carica subito. */
    private void onLyricsTrackChanged() {
        cachedLyricsUrl = null;
        if (lyricsFetchDisposable != null && !lyricsFetchDisposable.isDisposed()) {
            lyricsFetchDisposable.dispose();
        }
        if (lyricsOverlay != null) {
            if (lyricsOverlay.isVisible() && currentItem != null) {
                fetchLyrics(currentItem);
            } else {
                lyricsOverlay.clear();
            }
    ...
    private void fetchLyrics(@NonNull final PlayQueueItem item) {
        if (lyricsOverlay == null) {
            return;
        }
        final String url = item.getUrl();
        cachedLyricsUrl = url;
        lyricsOverlay.setLoading();
        if (lyricsClient == null) {
            lyricsClient = new LyricsClient(new OkHttpLyricsGetter(), LyricsClient.DEFAULT_ENDPOINT);
        }
        lyricsFetchDisposable = Observable.fromCallable(() ->
                        lyricsClient.fetchSyncedLines(item.getUploader(), item.getTitle(), null,
                                (int) Math.round(item.getDuration())))
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(lines -> {
                    if (!url.equals(cachedLyricsUrl)) {
                        return; // brano cambiato durante la fetch
                    }
                    ...
                    Log.e(TAG, "Lyrics fetch failed: " + error.getMessage(), error);
    ...
    public void onClick(final View v) {
        ...
        if (v.getId() == binding.resizeTextView.getId()) {
            ...
            onCaptionClicked();
        } else if (v.getId() == binding.lyricsButton.getId()) {
            onLyricsClicked();
        ...
        } else if (v.getId() == binding.castButton.getId()) {
            onCastClicked();
    ...
    //region DLNA cast (022-S2)

    /**
     * Apre il dialogo con la lista dispositivi DLNA (discovery SSDP via jupnp)
    ...
    public void onCastClicked() {
        ...
        manager.setDevicesListener(devices -> {
            if (castDialog != null && castDialog.isShowing() && !rebuildingCastDialog) {
                new Handler(Looper.getMainLooper()).post(() -> rebuildCastDialog(manager));
            }
        });
        manager.start();
        rebuildCastDialog(manager);
    }

    private void rebuildCastDialog(final DlnaDiscoveryManager manager) {
        final AppCompatActivity activity = getParentActivity();
        if (activity == null || activity.isFinishing()) {
            return;
        }
        if (castDialog != null) {
            rebuildingCastDialog = true;
            castDialog.dismiss();
            castDialog = null;
            rebuildingCastDialog = false;
        }
        final List<DlnaDevice> devices = manager.getDevices();
        final AlertDialog.Builder builder = new AlertDialog.Builder(activity)
                .setTitle(R.string.cast_title)
                .setOnDismissListener(dialog -> manager.stop())
                .setNegativeButton(R.string.cancel, (dialog, which) -> dialog.dismiss());
        if (devices.isEmpty()) {
            builder.setMessage(R.string.cast_no_devices);
            builder.setPositiveButton(R.string.cast_search_again, (dialog, which) -> {
                manager.search();
                rebuildCastDialogDelayed(manager);
            });
        } else {
            final String[] names = new String[devices.size()];
            for (int i = 0; i < devices.size(); i++) {
                names[i] = devices.get(i).getFriendlyName();
            }
            builder.setItems(names, (dialog, which) -> startCastToDevice(manager, devices.get(which)));
            builder.setNeutralButton(R.string.cast_search_again, (dialog, which) -> {
                manager.search();
                rebuildCastDialogDelayed(manager);
            });
        }
        castDialog = builder.show();
    }

    /** Dopo una nuova ricerca SSDP ridai il tempo ai dispositivi di rispondere, poi ricostruisci. */
    private void rebuildCastDialogDelayed(final DlnaDiscoveryManager manager) {
        new Handler(Looper.getMainLooper()).postDelayed(() -> rebuildCastDialog(manager), 1500);
    }

    private void startCastToDevice(final DlnaDiscoveryManager manager, final DlnaDevice device) {
        final AppCompatActivity activity = getParentActivity();
        if (activity == null) {
            return;
        }
        getCurrentStreamInfo().ifPresent(info -> {
            // DLNA: URL progressivo (MP4/WebM con audio) preferito, fallback HLS (D-2).
            // Gli stream video-only/SABR/DASH non sono adatti a un renderer TV.
            final List<String> progressive = info.getVideoStreams().stream()
                    .filter(stream -> stream.getDeliveryMethod() == DeliveryMethod.PROGRESSIVE_HTTP)
                    .map(Stream::getUrl)
                    .collect(Collectors.toList());
            final String streamUrl = DlnaStreamPicker.INSTANCE.pick(progressive, info.getHlsUrl());
            if (streamUrl == null) {
                Toast.makeText(context, R.string.cast_no_stream, Toast.LENGTH_LONG).show();
                return;
            }
            pause();
            manager.cast(device, streamUrl, (ok, error) -> {
                // Callback jupnp su thread di rete: posta sul main per la UI.
                new Handler(Looper.getMainLooper()).post(() -> {
                    final String message = ok
                            ? activity.getString(R.string.cast_started, device.getFriendlyName())
                            : activity.getString(R.string.cast_failed, error != null ? error : "unknown");
    ...
    //region Chromecast cast (022-S3)

    /**
     * Inizializza il casting Chromecast (022-S3): gate runtime su Google Play
     * Services (D-2 — l'app non ha product flavors) e, se superato, aggancia il
     * MediaRouteButton del player al route selector del Cast SDK e registra il
     * listener di sessione (trasporto della play queue all'avvio).
     */
    private void initChromecast() {
        if (!ChromecastCastManager.isPlayServicesAvailable(context)) {
            return;
        }
        final ChromecastCastManager manager = ChromecastCastManager.get(context);
        this.chromecastManager = manager;
        manager.setCastListener(new ChromecastCastManager.CastListener() {
            @Override
            public void onCastSessionStarted(final CastSession session) {
                // Callback su thread main (Cast SDK): trasporta la queue e mette
                // in pausa il player locale, come per DLNA (022-S2).
                transportPlayQueueToChromecast(session);
            }
            ...
            public void onCastSessionEnded() {
                ...
                Toast.makeText(context, R.string.cast_chromecast_unavailable, Toast.LENGTH_SHORT).show();
            }
        });
        manager.prepare();
        binding.chromecastButton.setRouteSelector(manager.getRouteSelector());
        binding.chromecastButton.setOnClickListener(v -> { /* il MediaRouteButton
            gestisce da solo chooser e controller (volume/play-pause) del Cast SDK */ });
    }

    /**
     * Trasporta la play queue sul ricevitore Chromecast: mappa ogni item della
     * queue in un input Cast (URL progressivo preferito, fallback HLS — D-2) e
     * la invia con queueLoad partendo dall'item corrente. Gli item senza URL
     * riproducibile vengono scartati (vedi CastQueueMapper).
     */
    private void transportPlayQueueToChromecast(final CastSession session) {
        ...
        // Risolvi le URL progressive/HLS per ogni item: l'attuale è già caricato
        // (StreamInfo in currentMetadata), gli altri via getStream() best-effort.
        // La queue viene inviata quando TUTTI gli item hanno concluso la risoluzione
        // (successo o errore): quelli falliti restano fuori, l'attuale entra sempre.
        final List<CastQueueInput> inputs = new ArrayList<>(items.size());
        final java.util.concurrent.atomic.AtomicInteger pending = new java.util.concurrent.atomic.AtomicInteger(items.size());
        for (int i = 0; i < items.size(); i++) {
            final PlayQueueItem item = items.get(i);
            final int index = i;
            if (index == currentIndex) {
                getCurrentStreamInfo().ifPresent(info -> inputs.add(toCastQueueInput(index, item, info)));
                if (pending.decrementAndGet() == 0) {
                    sendChromecastQueue(session, inputs, currentIndex);
                }
            } else {
                // Best-effort: se la risoluzione fallisce l'item viene scartato
                // (la queue parte comunque dall'item corrente).
                item.getStream()
                        .subscribeOn(Schedulers.io())
                        .subscribe(info -> {
                            synchronized (inputs) {
                                inputs.add(toCastQueueInput(index, item, info));
                            }
                            if (pending.decrementAndGet() == 0) {
                                sendChromecastQueue(session, inputs, currentIndex);
                            }
    ...
    private CastQueueInput toCastQueueInput(final int index, final PlayQueueItem item,
                                            final StreamInfo info) {
        final List<String> progressive = info.getVideoStreams().stream()
                .filter(stream -> stream.getDeliveryMethod() == DeliveryMethod.PROGRESSIVE_HTTP)
                .map(Stream::getUrl)
                .collect(Collectors.toList());
        return new CastQueueInput(
                index, item.getTitle(), item.getUrl(), item.getThumbnailUrl(),
                item.getDuration() * 1000L, progressive, info.getHlsUrl());
    }

    private void sendChromecastQueue(final CastSession session,
                                     final List<CastQueueInput> inputs,
                                     final int currentIndex) {
        // La risoluzione asincrona può concludersi su thread IO; il Cast SDK
        // richiede le chiamate sul main thread.
        new Handler(Looper.getMainLooper()).post(() -> {
            final AppCompatActivity activity = getParentActivity();
            if (activity == null || chromecastManager == null || inputs.isEmpty()) {
                return;
            }
            final List<CastQueueEntry> entries = CastQueueMapper.INSTANCE.map(inputs);
            pause();
            final long position = simpleExoPlayer != null ? simpleExoPlayer.getCurrentPosition() : 0L;
            final boolean sent = chromecastManager.transportQueue(entries, currentIndex, position);
            final String message = sent
                    ? activity.getString(R.string.cast_chromecast_started, "Chromecast")
                    : activity.getString(R.string.cast_chromecast_no_stream);
            Toast.makeText(context, message, Toast.LENGTH_LONG).show();
        });
    }
    //endregion
    ...
    /*//////////////////////////////////////////////////////////////////////////
    // Watch together (022-S16)
    //////////////////////////////////////////////////////////////////////////*/
    //region Watch together

    /**
     * Apre il flusso watch-together: se è già in corso una sessione mostra il
    ...
    public void onWatchTogetherClicked() {
        ...
        new AlertDialog.Builder(activity)
                .setTitle(R.string.watch_together)
                .setItems(new String[]{
                        activity.getString(R.string.watch_together_create),
                        activity.getString(R.string.watch_together_join)}, (dialog, which) -> {
                    if (which == 0) {
                        createWatchTogetherRoom();
                    } else {
                        showJoinWatchTogetherDialog();
                    }
                })
    ...
    public void createWatchTogetherRoom() {
        ...
        final WatchTogetherServer server =
                new WatchTogetherServer(WatchTogetherProtocol.DEFAULT_PORT,
                        WatchTogetherProtocol.DEFAULT_MAX_PARTICIPANTS);
        try {
            server.start();
        } catch (final Exception e) {
            Toast.makeText(context, context.getString(R.string.watch_together_server_failed,
                    e.getMessage() != null ? e.getMessage() : e.getClass().getSimpleName()),
                    Toast.LENGTH_LONG).show();
            return;
        }
        final List<String> ips = server.localIpAddresses();
        final String lanIp = ips.isEmpty() ? "127.0.0.1" : ips.get(0);
        startWatchTogetherSession(roomId, "127.0.0.1", server);
        Toast.makeText(context,
                context.getString(R.string.watch_together_created, roomId, lanIp),
                Toast.LENGTH_LONG).show();
    }

    /** Dialogo per unirsi a una stanza esistente (IP host + codice stanza). */
    private void showJoinWatchTogetherDialog() {
        final AppCompatActivity activity = getParentActivity();
        if (activity == null || activity.isFinishing()) {
            return;
        }
        ...
        final int pad = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 16,
                context.getResources().getDisplayMetrics());
        layout.setPadding(pad, pad / 2, pad, 0);

        final EditText ipInput = new EditText(activity);
        ipInput.setHint(R.string.watch_together_host_ip);
        ipInput.setInputType(InputType.TYPE_CLASS_PHONE);
        layout.addView(ipInput);

        final EditText codeInput = new EditText(activity);
        codeInput.setHint(R.string.watch_together_room_code);
        codeInput.setInputType(InputType.TYPE_CLASS_TEXT
                | InputType.TYPE_TEXT_FLAG_CAP_CHARACTERS);
        layout.addView(codeInput);

        final EditText portInput = new EditText(activity);
        portInput.setHint(R.string.watch_together_port);
        portInput.setInputType(InputType.TYPE_CLASS_NUMBER);
        portInput.setText(String.valueOf(WatchTogetherProtocol.DEFAULT_PORT));
        layout.addView(portInput);

        new AlertDialog.Builder(activity)
                .setTitle(R.string.watch_together_join)
                .setView(layout)
                .setPositiveButton(R.string.watch_together_join, (dialog, which) -> {
                    final String ip = ipInput.getText().toString().trim();
                    final String code = codeInput.getText().toString().trim();
                    if (ip.isEmpty() || code.isEmpty()) {
                        Toast.makeText(context, R.string.watch_together_join_failed,
                                Toast.LENGTH_LONG).show();
                        return;
                    }
    ...
    /**
     * Crea client + sessione e la avvia. [server] è non-null solo quando questo
     * dispositivo ha appena creato la stanza (e quindi ne è l'host di fatto).
     */
    private void startWatchTogetherSession(final String roomId, final String host,
                                           @Nullable final WatchTogetherServer server) {
        final String participantId = UUID.randomUUID().toString().replace("-", "").substring(0, 8);
        final String displayName = (Build.MODEL + "-" + participantId.substring(0, 4))
                .substring(0, Math.min(Build.MODEL.length() + 5, 20));
        final WatchTogetherClient client =
                new WatchTogetherClient(host, WatchTogetherProtocol.DEFAULT_PORT, roomId,
                        participantId, displayName);
        final WatchTogetherSession session = new WatchTogetherSession(roomId, participantId,
                displayName, server, client);
        session.setListener(watchTogetherListener);
        watchTogetherSession = session;
        session.start();
        updateWatchTogetherBar(0, server != null, true);
    }

    /** Listener della sessione: i callback arrivano dal thread WebSocket, posta sul main. */
    private final WatchTogetherSession.Listener watchTogetherListener =
            new WatchTogetherSession.Listener() {
                @Override
                public void onSessionUpdated(final List<Participant> participants,
                                             final boolean isHost, final boolean connected) {
                    postToMain(() -> updateWatchTogetherBar(participants.size(), isHost, connected));
                }

                @Override
                public void onRemoteSnapshot(final PlaybackSnapshot snapshot) {
                    postToMain(() -> applyRemoteSnapshot(snapshot));
                }

                @Override
                public void onRemoteSeek(final long positionMs) {
                    postToMain(() -> {
                        if (!exoPlayerIsNull()) {
                            seekTo(positionMs);
                        }
                ...
                public void onError(final String code, final String message) {
                    postToMain(() -> {
                        if (watchTogetherSession != null) {
                            Toast.makeText(context,
                                    context.getString(R.string.watch_together_join_failed, message),
                                    Toast.LENGTH_LONG).show();
                        }
                    });
                }
                ...
                public void onDisconnected(final String reason) {
                    postToMain(() -> {
                        if (watchTogetherSession != null) {
                            Toast.makeText(context, R.string.watch_together_disconnected,
                                    Toast.LENGTH_SHORT).show();
                            updateWatchTogetherBar(0, false, false);
                        }
                    });
                }
            };
    ...
    /**
     * Applica uno snapshot remoto allo stato locale (solo slave): decide con il
     * WatchSyncEngine se seek/play/pause e non tocca mai la riproduzione locale
     * quando l'host sta riproducendo un brano diverso.
     */
    private void applyRemoteSnapshot(final PlaybackSnapshot snapshot) {
        if (watchTogetherSession == null || exoPlayerIsNull() || currentItem == null) {
            return;
        }
        final WatchSyncAction action = WatchSyncEngine.INSTANCE.evaluate(
                snapshot,
                Math.max((int) simpleExoPlayer.getCurrentPosition(), 0),
                getPlayWhenReady(),
                currentItem.getUrl(),
                System.currentTimeMillis());
        if (action instanceof WatchSyncAction.MediaMismatch) {
            binding.watchTogetherStatusText.setText(R.string.watch_together_media_mismatch);
            return;
        }
        if (action instanceof WatchSyncAction.SeekTo) {
            seekTo(((WatchSyncAction.SeekTo) action).getPositionMs());
        } else if (action instanceof WatchSyncAction.SeekAndSetPlaying) {
            final WatchSyncAction.SeekAndSetPlaying both =
                    (WatchSyncAction.SeekAndSetPlaying) action;
            seekTo(both.getPositionMs());
            applyRemotePlaying(both.getPlaying());
        } else if (action instanceof WatchSyncAction.SetPlaying) {
            applyRemotePlaying(((WatchSyncAction.SetPlaying) action).getPlaying());
        }
    }

    private void applyRemotePlaying(final boolean playing) {
        if (exoPlayerIsNull()) {
            return;
        }
        if (playing) {
    ...
    /**
     * Pubblica lo stato di riproduzione corrente (solo host; il throttle è nel
     * WatchTogetherSession via WatchSyncEngine.shouldPublish). No-op se non in
     * sessione, se non host, o se il player non è pronto.
     */
    private void publishWatchTogetherTick() {
        if (watchTogetherSession == null || !watchTogetherSession.isHost()
                || exoPlayerIsNull() || currentItem == null) {
            return;
        }
        watchTogetherSession.publishSnapshot(
                getPlayWhenReady(),
                Math.max((int) simpleExoPlayer.getCurrentPosition(), 0),
                simpleExoPlayer.getPlaybackParameters().speed,
    ...
    /** Pubblica un seek esplicito (solo host). */
    private void publishWatchTogetherSeek(final long positionMillis) {
        if (watchTogetherSession != null && watchTogetherSession.isHost()
                && watchTogetherSession.getConnected()) {
            watchTogetherSession.publishSeek(positionMillis);
        }
    }

    /** Aggiorna la barra della stanza: codice, partecipanti, ruolo, stato connessione. */
    private void updateWatchTogetherBar(final int participantCount, final boolean isHost,
                                        final boolean connected) {
        if (binding == null || watchTogetherSession == null) {
            return;
        }
        binding.watchTogetherBar.setVisibility(connected ? View.VISIBLE : View.GONE);
        if (!connected) {
            return;
        }
        final String role = context.getString(isHost
                ? R.string.watch_together_host : R.string.watch_together_guest);
        binding.watchTogetherStatusText.setText(context.getString(
                R.string.watch_together_status, watchTogetherSession.getRoomId(),
                participantCount, role));
    }

    /** Dialogo della stanza attiva: lista partecipanti + esci. */
    private void showWatchTogetherSessionDialog() {
        final AppCompatActivity activity = getParentActivity();
        if (activity == null || activity.isFinishing() || watchTogetherSession == null) {
            return;
        }
        final List<Participant> participants = watchTogetherSession.getParticipants();
        final String[] names = new String[participants.size()];
        for (int i = 0; i < participants.size(); i++) {
            final Participant p = participants.get(i);
            names[i] = p.getDisplayName() + (p.isHost() ? " ★" : "");
        }
        new AlertDialog.Builder(activity)
                .setTitle(R.string.watch_together_participants)
                .setItems(names, null)
                .setPositiveButton(R.string.watch_together_leave,
                        (dialog, which) -> leaveWatchTogetherRoom())
                .setNegativeButton(R.string.cancel, null)
                .show();
    }
    ...
    /** Ferma la sessione e pulisce la UI; usato anche nel destroy() senza toast. */
    private void stopWatchTogetherSession() {
        if (watchTogetherSession == null) {
            return;
        }
        watchTogetherSession.stop();
        watchTogetherSession = null;
        if (watchTogetherDialog != null) {
            watchTogetherDialog.dismiss();
            watchTogetherDialog = null;
        }
        if (binding != null) {
            binding.watchTogetherBar.setVisibility(View.GONE);
        }
    }

    /** Posta un runnable sul main thread (i callback WS arrivano da altro thread). */
    private void postToMain(final Runnable runnable) {
