...
public final class ThemeHelper {
    ...
    /**
     * Return true if the user selected the "black" theme variant, either explicitly
     * or as the night variant of the automatic theme (coherent with {@link #getThemeForService}).
     *
     * @param context context to get the selected theme
     * @return whether the black theme is selected
     */
    public static boolean isBlackTheme(final Context context) {
        final Resources res = context.getResources();
        final String blackThemeKey = res.getString(R.string.black_theme_key);
        final String selectedThemeKey = getSelectedThemeKey(context);
        if (selectedThemeKey.equals(blackThemeKey)) {
            return true;
        }
        if (selectedThemeKey.equals(res.getString(R.string.auto_device_theme_key))
                && isDeviceDarkThemeEnabled(context)) {
            return getSelectedNightThemeKey(context).equals(blackThemeKey);
        }
        return false;
    }
    ...
    public static boolean shouldUseExperimentalNewUi(final Context context) {
        final SharedPreferences preferences = PreferenceManager.getDefaultSharedPreferences(context);
        return preferences.getBoolean(context.getString(R.string.use_experimental_new_ui_key), true);
    }
